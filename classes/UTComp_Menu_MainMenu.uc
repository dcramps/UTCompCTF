...
simulated function SaveSettings()
{
  ...
  Log("Saving settings");
  settings = PlayerOwner().Spawn(class'UTComp_Settings');
  settings.saveClientConfig();
}

simulated function SaveHUDSettings()
{
  local UTComp_HUDSettings settings;

  Log("Saving HUD settings");
  settings = PlayerOwner().Spawn(class'UTComp_HUDSettings');
  settings.saveClientConfig();
}
...
function bool InternalOnClick(GUIComponent C)
{
    if(C==UTCompMenuButtons[0])
        PlayerOwner().ClientReplaceMenu("UTCompCTFv03.UTComp_Menu_BrightSkins");

    else if(C==UTCompMenuButtons[1])
        PlayerOwner().ClientReplaceMenu("UTCompCTFv03.UTComp_Menu_ColorNames");

    else if(C==UTCompMenuButtons[2])
        PlayerOwner().ClientReplaceMenu("UTCompCTFv03.UTComp_Menu_TeamOverlay");

    else if(C==UTCompMenuButtons[3])
        PlayerOwner().ClientReplaceMenu("UTCompCTFv03.UTComp_Menu_Crosshairs");

    else if(C==UTCompMenuButtons[4])
        PlayerOwner().ClientReplaceMenu("UTCompCTFv03.UTComp_Menu_Hitsounds");

    else if(C==UTCompMenuButtons[5])
        PlayerOwner().ClientReplaceMenu("UTCompCTFv03.UTComp_Menu_Voting");

    else if(C==UTCompMenuButtons[6])
        PlayerOwner().ClientReplaceMenu("UTCompCTFv03.UTComp_Menu_AutoDemoSS");

    else if(C==UTCompMenuButtons[7])
        PlayerOwner().ClientReplaceMenu("UTCompCTFv03.UTComp_Menu_Miscellaneous");
...
defaultproperties
{
     ...
     End Object
     c_Main=GUITabControl'UTCompCTFv03.UTComp_Menu_MainMenu.LoginMenuTC'
     ...
     Begin Object Class=FloatingImage Name=FloatingFrameBackground
         ...
         DropShadowY=0
     End Object
     i_FrameBG=FloatingImage'UTCompCTFv03.UTComp_Menu_MainMenu.FloatingFrameBackground'
     ...
     Begin Object Class=FloatingImage Name=FloatingFrameBackground2
         ...
         DropShadowY=0
     End Object
     i_FrameBG2=FloatingImage'UTCompCTFv03.UTComp_Menu_MainMenu.FloatingFrameBackground2'
